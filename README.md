...
```
composer require semy/cassandra
```
...
```php
semy\Cassandra\CassandraServiceProvider::class,
```
...
```php
$app->register('semy\Cassandra\CassandraServiceProvider');
...
```php
use semy\Cassandra\Eloquent\Model as Eloquent;
...
```php
'Moloquent'       => 'semy\Cassandra\Eloquent\Model',
```
...
```php
'semy\Cassandra\Auth\PasswordResetServiceProvider',
```
...
```php
use semy\Cassandra\Eloquent\SoftDeletes;
...
*There is also support for upsert operations, check https://github.com/semy/laravel-cassandra#cassandra-specific-operations*
...
If you're using a hybrid Cassandra and SQL setup, you're in luck! The model will automatically return a Cassandra- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `semy\Cassandra\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.

Example SQL-based User model:

```php
use semy\Cassandra\Eloquent\HybridRelations;
